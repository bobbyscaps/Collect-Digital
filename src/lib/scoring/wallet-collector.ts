...
export interface CollectorSubScores {
  holdingBehavior: number;
  collectionQuality: number;
  profitabilityTiming: number;
...
export const COLLECTOR_SUB_SCORE_LABELS: Record<keyof CollectorSubScores, string> = {
  ...
  profitabilityTiming: "Profitability & Timing",
  activityLevel: "Activity Level",
  communityParticipation: "Community Participation",
  walletLongevity: "Wallet Longevity",
};

/** The six normalized (0-100) sub-scores that feed the Collector Score. */
export function computeCollectorSubScores(
  metrics: WalletBehaviorMetrics
): CollectorSubScores {
  return {
    holdingBehavior: clamp(
      metrics.heldOver90Pct * 0.35 +
        metrics.heldOver180Pct * 0.35 +
        (metrics.avgHoldDays / 365) * 30
    ),
    collectionQuality: clamp(metrics.collectionQualityScore * 100),
    profitabilityTiming: clamp(
      (metrics.realizedPnlEth ?? 0) * 1.2 +
        metrics.prePumpBuyRate * 40 +
        metrics.sellIntoPumpRate * 25
    ),
    activityLevel: clamp(
      (metrics.boughtCount + metrics.soldCount) / 3 +
        metrics.saleFrequencyPerMonth * 12
    ),
    communityParticipation: clamp(metrics.communityParticipationScore * 100),
    walletLongevity: clamp((metrics.walletAgeDays / 3650) * 100),
  };
}
